  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-display font-extrabold text-white">Application Vault</h1>
          <p className="text-gray-400 text-xs mt-1">Publish, update, and manage versions of NEXVORA applications.</p>
        </div>
        <button
      ...
      {loading && apps.length === 0 ? (
        <div className="text-center py-12">
          ...
          <span className="text-gray-400 text-sm">Synchronizing app inventory...</span>
        </div>
      ) : apps.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-sm">
              ...
              <tbody className="divide-y divide-space-border/40 text-gray-300">
                {apps.map((app) => (
                  <tr key={app._id} className="hover:bg-space-dark/20 transition-colors">
                    ...
                    <td className="p-5">
                      <div className="flex items-center space-x-3">
                        <img
                          ...
                          alt={app.name}
                          className="h-10 w-10 rounded-lg object-contain bg-space-dark p-1 border border-space-border"
                        />
                        <div>
                          <span className="text-white font-bold font-display block">{app.name}</span>
                    ...
                    {/* Category */}
                    <td className="p-5">
                      <span className="text-xs bg-space-dark/80 text-gray-300 border border-space-border/80 px-2.5 py-0.5 rounded-full">
                        {app.category}
                      </span>
                    </td>

                    {/* Version */}
                    <td className="p-5 font-mono text-xs text-gray-400">v{app.version}</td>

                    {/* Size */}
                    <td className="p-5 text-gray-400">{app.size || 'N/A'}</td>

                    {/* Downloads */}
                    <td className="p-5 font-mono text-xs text-neon-blue">{(app.downloadCount || 0).toLocaleString()}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="glass-panel max-w-2xl w-full rounded-2xl border border-neon-blue/35 p-6 md:p-8 relative space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            {/* Modal Title */}
            <h2 className="text-white font-extrabold text-2xl font-display uppercase tracking-wide border-b border-space-border/60 pb-3">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div className="space-y-1">
                  <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Application Name</label>
                  <input
                    type="text"
                    required
                    value={name}
                ...
                {/* Category selector */}
                <div className="space-y-1">
                  <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Category</label>
                  <select
                    value={category}
                    onChange={(e) => setCategory(e.target.value)}
                    className="w-full bg-space-dark border border-space-border focus:border-neon-blue focus:outline-none rounded-xl py-3.5 px-4 text-sm text-white cursor-pointer"
                  >
                    {categories.length === 0 ? (
                      <option value="" disabled className="bg-space-dark text-white">No categories available (Please seed database)</option>
                    ) : (
                      <>
                        <option value="" disabled className="bg-space-dark text-white">Select a Category</option>
                        {categories.map(c => (
                          <option key={c._id} value={c.name} className="bg-space-dark text-white">{c.name}</option>
                        ))}
                      </>
                    )}
                  </select>
              ...
              {/* Descriptions */}
              <div className="space-y-1">
                <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Short Tagline Description</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Detailed Description Overview</label>
                <textarea
                  rows={3}
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="w-full bg-space-dark border border-space-border focus:border-neon-blue focus:outline-none rounded-xl py-3 px-4 text-sm text-gray-100 placeholder-gray-500 resize-none"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div className="space-y-1">
                  <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Version String</label>
                  <input
                    type="text"
                    required
                    value={version}
                ...
                <div className="space-y-1">
                  <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">File Size</label>
                  <input
                    type="text"
                    value={size}
                ...
                <div className="space-y-1">
                  <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Rating Value</label>
                  <input
                    type="number"
                    step="0.1"
                    min="1"
              ...
              <div className="space-y-1">
                <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Features Listing (one per line)</label>
                <textarea
                  rows={3}
                  value={featuresText}
                  onChange={(e) => setFeaturesText(e.target.value)}
                  className="w-full bg-space-dark border border-space-border focus:border-neon-blue focus:outline-none rounded-xl py-3 px-4 text-sm text-gray-100 placeholder-gray-500 font-sans"
              ...
              {modalType === 'edit' && (
                <div className="space-y-1 p-3 bg-neon-blue/5 border border-neon-blue/15 rounded-xl">
                  <label className="text-neon-blue text-xs font-semibold uppercase tracking-wider">Changelog Release Notes</label>
                  <input
                    type="text"
                    value={changelogNotes}
                    onChange={(e) => setChangelogNotes(e.target.value)}
                    className="w-full bg-space-dark border border-space-border focus:border-neon-blue focus:outline-none rounded-xl py-2.5 px-4 text-sm text-gray-100 placeholder-gray-500 mt-1"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
                <div className="space-y-1">
                  <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">GitHub Release APK URL</label>
                  <input
                    type="url"
                    required
                ...
                <div className="space-y-1">
                  <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Release Notes (This Version)</label>
                  <input
                    type="text"
                    value={releaseNotes}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
                ...
                <div className="space-y-2">
                  <span className="text-gray-400 text-xs font-semibold uppercase tracking-wider block">App Logo</span>
                  <input
                    type="file"
                    accept="image/*"
                    onChange={handleIconChange}
                    className="text-xs text-gray-500 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-space-dark file:text-gray-300 file:cursor-pointer hover:file:bg-space-dark/70"
                  />
                </div>

                {/* Screenshots */}
                <div className="space-y-2">
                  <span className="text-gray-400 text-xs font-semibold uppercase tracking-wider block">Screenshots (max 6)</span>
                  <input
                    type="file"
                    multiple
                    accept="image/*"
              ...
              <div className="flex items-center space-x-6 border-t border-space-border/40 pt-4">
              ...
              {isUploading && (
                <div className="flex flex-col items-center justify-center p-3.5 rounded-xl bg-neon-blue/10 border border-neon-blue/20 text-xs text-neon-blue font-semibold animate-pulse text-center">
              ...
              <button
                ...
                className="w-full text-center text-sm font-semibold text-space-darkest bg-gradient-to-r from-neon-blue to-blue-500 hover:brightness-110 shadow-neon-glow py-3.5 rounded-xl transition-all disabled:opacity-50"
              >
