  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-display font-extrabold text-white">Categories Management</h1>
          <p className="text-gray-400 text-xs mt-1">Configure categories to classify application downloads.</p>
        </div>
        <button
      ...
      {loading && categories.length === 0 ? (
        <div className="text-center py-12">
          ...
          <span className="text-gray-400 text-sm">Loading category nodes...</span>
        </div>
      ) : categories.length === 0 ? (
        ...
        <div className="glass-panel rounded-2xl border border-space-border/60 overflow-hidden">
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody className="divide-y divide-space-border/40 text-gray-300">
              {categories.map((cat) => (
                <tr key={cat._id} className="hover:bg-space-dark/25 transition-colors">
                  <td className="p-5 font-bold text-white flex items-center space-x-2.5">
                    <Folder className="h-4.5 w-4.5 text-neon-blue" />
                    <span>{cat.name}</span>
                  </td>
                  <td className="p-5 font-mono text-xs text-gray-500">{cat.slug}</td>
                  <td className="p-5 text-xs text-gray-400 max-w-sm leading-relaxed">{cat.description || 'No description provided.'}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-panel max-w-md w-full rounded-2xl border border-neon-blue/35 p-6 md:p-8 relative space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Category Name</label>
                <input
                  type="text"
                  required
                  value={name}
